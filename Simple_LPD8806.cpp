...
Simple_LPD8806::Simple_LPD8806(uint16_t n, uint8_t dpin, uint8_t cpin) {
  pixels = NULL;
  begun  = false;
  updateLength(n);
...
void Simple_LPD8806::begin(void) {
...
void Simple_LPD8806::updatePins(uint8_t dpin, uint8_t cpin) {
...
void Simple_LPD8806::startBitbang() {
...
void Simple_LPD8806::updateLength(uint16_t n) {
...
uint16_t Simple_LPD8806::numPixels(void) {
  return numLEDs;
}
...
uint32_t Simple_LPD8806::Color(byte r, byte g, byte b) {
...
void Simple_LPD8806::setPixelColor(uint16_t n, uint8_t r, uint8_t g, uint8_t b) {
...
void Simple_LPD8806::setPixelColor(uint16_t n, uint32_t c) {
  if(n < numLEDs) { // Arrays are 0-indexed, thus NOT '<='
    uint8_t *p = &pixels[n * 3];
    *p++ = (c >> 16) | 0x80;
    *p++ = (c >>  8) | 0x80;
...
uint32_t Simple_LPD8806::getPixelColor(uint16_t n) {
  if(n < numLEDs) {
    uint16_t ofs = n * 3;
